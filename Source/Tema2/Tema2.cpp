...
void Tema2::RenderMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, Texture2D* texture1)
{
	...
	glUniform3f(light_position, this->light_position.x + 0.25f, this->light_position.y, this->light_position.z);

	int light_direction = glGetUniformLocation(shader->program, "light_direction");
	glUniform3f(light_direction, this->light_direction.x, this->light_direction.y, this->light_direction.z);

	// TODO: Set eye position (camera position) uniform
	glm::vec3 eyePosition = this->camera->GetPosition();
	...
	int spot = glGetUniformLocation(shader->program, "is_spot");
	glUniform1i(spot, this->is_spot);

	int angle = glGetUniformLocation(shader->program, "cut_off_angle");
	glUniform1f(angle, this->cut_off_angle);
...
void Tema2::update_light_direction() {
	/*float xp = this->light_direction.x * cos(RADIANS(1)) + this->light_direction.y * sin(RADIANS(1));
	float yp = -this->light_direction.x * sin(RADIANS(1)) + this->light_direction.y * cos(RADIANS(1));
	float zp = this->light_direction.z;

	this->light_direction = glm::vec3(xp, yp, zp);*/
	glm::mat4 modelMatrix = glm::mat4(1);
	modelMatrix = glm::rotate(modelMatrix, RADIANS(10.0f), this->light_direction + glm::vec3(0, 0.01f, 0));
	glm::vec4 new_pos = modelMatrix * glm::vec4(this->light_direction, 1);
	this->light_direction.x = new_pos.x; this->light_direction.y = new_pos.y; this->light_direction.z = new_pos.z;
...
void Tema2::update_decor() {
	...
	if (right_decor[0]->getPosition().z < DECOR_Z_MIN) {
		...
		if (right_decor[0]->getName() == "punctiform_lampost") {
			...
			this->light_position = glm::vec3(1, -1, 0);
			glm::vec3 lampPost_position =
				glm::vec3(rand() % (RIGHT_DECORE_MIN_X - RIGHT_DECORE_MAX_X) + RIGHT_DECORE_MAX_X,
					LAMP_POST_DIMENSON.y * LAMP_POST_SCALE_RATIO.y / 2 - TILE_DIMENSION.y,
